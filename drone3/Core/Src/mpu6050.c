...
SD_MPU6050_Result SD_MPU6050_ReadAccelerometer(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	while(HAL_I2C_Master_Transmit(Handle, (uint16_t)address, &reg, 1, 100) != HAL_OK);

	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 6, 100) != HAL_OK);
...
SD_MPU6050_Result SD_MPU6050_ReadGyroscope(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 6, 1) != HAL_OK);
