...
#define MS5611_ADDRESS                0xEE  ///0x77
#define MS5611_ADDRESS_READ           0xEF  ///0x77

#define MS5611_CMD_ADC_READ           0x00
#define MS5611_CMD_RESET              0x1E
#define MS5611_CMD_CONV_D1            0x40
#define MS5611_CMD_CONV_D2            0x50
#define MS5611_CMD_READ_PROM          0xA0   //0xA2
...
typedef struct {
 ...
 //public:
	...
	bool begin(MS5611 *dev, ms5611_osr_t osr); // = MS5611_HIGH_RES);
	uint32_t readRawTemperature(MS5611 *dev, uint8_t y);
	uint32_t readRawPressure(MS5611 *dev, uint8_t x);
	double readTemperature(MS5611* dev, bool compensation, uint8_t y); // = false);
	int32_t readPressure(MS5611* dev, bool compensation, uint8_t x, uint8_t y); // = false);
