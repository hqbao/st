/* 
   Collection of procedures for communication with a pair of ADNS3080 sensors.
   There is a procedure for each sensor of the pair, so there are two versions of each function.
   Needed materials include:
   Copyright (c) Nikola Milenic, Nemanja Filipovic @ Petnica Science Center 2014 http://ele.petnica.rs/?page_id=457
*/
#include "stm32h7xx_hal.h"
#include "optical_flow.h"
#include "stdbool.h"
#include "kalman.h"
//#include "srom.h"
extern SPI_HandleTypeDef hspi2;
const unsigned char SROM[1986]={

};
//////////////////////////////
uint8_t O_F_Read(uint8_t reg){
	MOUSECAM_CS_LOW;
	uint8_t buff[2];
	buff[0] = reg;
	while(HAL_SPI_Transmit(&hspi2, (uint8_t*) &buff[0], 1, 10)!=HAL_OK){;}
	HAL_Delay(1);
	while(HAL_SPI_Receive(&hspi2, (uint8_t*) &buff[1], 1, 10)!=HAL_OK){;}	
	MOUSECAM_CS_HIG;
	return buff[1];
}
//////////////////////////////
void O_F_Write(uint8_t reg, uint8_t val){
	MOUSECAM_CS_LOW;
	uint8_t buff[2];
	buff[0] = reg|0x80;
	buff[1] = val;
	while(HAL_SPI_Transmit(&hspi2, (uint8_t*) &buff[0], 1, 10)!=HAL_OK){;}	
	HAL_Delay(1);
	while(HAL_SPI_Transmit(&hspi2, (uint8_t*) &buff[1], 1, 10)!=HAL_OK){;}		
	MOUSECAM_CS_HIG;
}
///////////////////////////////////////////////////
void burst_read(ADNS3080_str* temp){ // Function for reading several registers related to motion. For the detailed description see page 16 of the datasheet
 int8_t buff[7];
 MOUSECAM_CS_LOW;
 //HAL_Delay(1);
 //////////////////////////////
 buff[0] = ADNS3080_MOTION_BURST;
 while(HAL_SPI_Transmit(&hspi2, (uint8_t*) &buff[0], 1, 10)!=HAL_OK){;}
 //HAL_Delay(1);
 while(HAL_SPI_Receive(&hspi2, (uint8_t*) buff, 3, 10)!=HAL_OK){;}
 temp->motion = buff[0];
 temp->dx = buff[1];
 temp->dy = buff[2];
 temp->x_pos += temp->dx;
 temp->y_pos += temp->dy;
// temp->surface_qual =  buff[3];
// temp->Shutter_Upper_ =  buff[4];
// temp->Shutter_Lower_ =  buff[5];
// temp->Maximum_px =  buff[6];
 MOUSECAM_CS_HIG;
}
//////////////////////////////////////////////
void init_sensor(void){ // sensor initialization
 char regVal;
 while(O_F_Read(ADNS3080_PRODUCT_ID) != 0x17){;}
 //locking the framerate
 regVal = O_F_Read(ADNS3080_EXTENDED_CONFIG);
 HAL_Delay(50);
 regVal = (regVal & ~0x01) | 0x01;
 O_F_Write(ADNS3080_EXTENDED_CONFIG, regVal);
 HAL_Delay(50);
 //setting the framerate to 2000FPs
 O_F_Write(ADNS3080_FRAME_PERIOD_MAX_BOUND_LOWER,0xE0);
 HAL_Delay(50);
 O_F_Write(ADNS3080_FRAME_PERIOD_MAX_BOUND_UPPER,0x2E);
 HAL_Delay(50);
 //setting the resolution to 1600Px
 regVal = O_F_Read(ADNS3080_CONFIGURATION_BITS);
 HAL_Delay(50);
 regVal |= 0x10;
 O_F_Write(ADNS3080_CONFIGURATION_BITS, regVal);
 HAL_Delay(50);
 Write_SROM();
}
void Write_SROM(void){
	MOUSECAM_CS_LOW; 
  O_F_Write(0x20, 0x44);
  HAL_Delay(50);
  O_F_Write(0x23, 0x07);
  HAL_Delay(50);
  O_F_Write(0x24, 0x88);
  HAL_Delay(50);
  O_F_Write(ADNS3080_SROM_ENABLE, 0x18);
  for(int16_t i=0; i<=1985; i++){
     O_F_Write(0x60, SROM[i]);
     HAL_Delay(1);// 10us 
		 }
  MOUSECAM_CS_HIG;
}
//////////////////////////////////////////////////
//////////////////////////////////////////////////
//////////////////////////////////////////////////

